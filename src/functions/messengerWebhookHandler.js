...
Deno.serve(async (req) => {
  try {
    ...
    if (!brandSlug || !webhookToken) {
      return Response.json({ response: 'Invalid request parameters' }, { status: 400 });
    }
    ...
    if (webhooks.length === 0) {
      return Response.json({ response: 'Invalid webhook token' }, { status: 401 });
    }
    ...
    if (brands.length === 0 || brands[0].slug !== brandSlug) {
      return Response.json({ response: 'Unauthorized' }, { status: 403 });
    }
    ...
    if (!from || !body) {
      return Response.json({ response: 'Invalid payload' }, { status: 400 });
    }
    ...
    try {
      ...
      if (kbEntries.length > 0 || brand.ai_instructions) {
